# this code is meant ot match polymers by information contained in various databases, after being canonicalized
...
df = pd.read_csv('/home/matt/Proj/MatSciCuration_TBV/Data/Polymers/PolymerData_PSMILES_canonicalized.csv')


# Group by the 'PSMILES' column and combine the rows using the 'first' function to fill NaNs with available values
combined_df = df.groupby('PSMILES').agg(lambda x: x.first_valid_index()).reset_index()
...
sorted_df.to_csv('/home/matt/Proj/MatSciCuration_TBV/Data/Polymers/PolymerData_Matched.csv',index=False)